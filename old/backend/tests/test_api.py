...
class TestRoot:
    def test_root(self):
        ...
        data = response.json()
        assert data["message"] == "Patterns API"
...
class TestMeasurementEndpoints:
    ...
    def test_list_individuals(self):
        response = client.get("/api/measurements/individuals")
        assert response.status_code == 200
        names = response.json()
        assert "vivien" in names
        assert "kwama" in names

    def test_get_individual_vivien(self):
        response = client.get("/api/measurements/individuals/vivien")
        assert response.status_code == 200
        data = response.json()
        assert data["full_bust"] == 102.0

    def test_get_individual_not_found(self):
        response = client.get("/api/measurements/individuals/unknown")
        assert response.status_code == 404


class TestPatternEndpoints:
    def test_list_pattern_types(self):
        response = client.get("/api/patterns/types")
        assert response.status_code == 200
        types = response.json()
        assert len(types) == 3
    ...
    def test_generate_baby_dress(self):
        response = client.post("/api/patterns/generate", json={
