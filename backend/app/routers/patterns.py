...
from app.patterns.corset import (
    CorsetMeasurements,
    ControlParameters as CorsetControlParameters,
    CorsetPattern,
)
from app.patterns.sleeve import (
    SleeveMeasurements,
...
PATTERN_TYPE_INFO: dict[PatternType, PatternTypeInfo] = {
    ...
    PatternType.baby_dress: PatternTypeInfo(
        name="baby_dress",
        label="Baby Dress (18 months)",
        required_measurements=[
            MeasurementFieldDefinition(name="TotalLength", description="Total length"),
            MeasurementFieldDefinition(name="ArmholeDepth", description="Armhole depth"),
            MeasurementFieldDefinition(name="ChestHalfWidth", description="Chest half width"),
            MeasurementFieldDefinition(name="HemHalfWidth", description="Hem half width"),
            MeasurementFieldDefinition(name="NeckWidthStart", description="Neck width start"),
            MeasurementFieldDefinition(name="StrapWidth", description="Strap width"),
            MeasurementFieldDefinition(name="FrontNeckDrop", description="Front neck drop"),
            MeasurementFieldDefinition(name="BackNeckDrop", description="Back neck drop"),
        ],
        control_parameters=[],
...
def _build_corset(req: PatternRequest):
    ...
    # Build FullMeasurements from the provided measurement dict
...
def generate_pattern(req: PatternRequest):
    ...
    try:
        if req.pattern_type == PatternType.baby_dress:
            # Baby dress uses its own Params dataclass
            params = BabyDressParams(**req.measurements)
            svg_content = generate_svg_string(params)

            if req.output_format == OutputFormat.svg:
                return Response(content=svg_content, media_type="image/svg+xml")

            # Baby dress only produces SVG, no construction/pattern split
            if req.output_format == OutputFormat.all:
                return PatternResponse(
                    construction_svg=svg_content,
                    pattern_svg=svg_content,
                    warnings=[],
                )

            raise HTTPException(status_code=400, detail="Baby dress only supports SVG output")
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            # Capture warnings from pattern construction
